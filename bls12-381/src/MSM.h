...
// Utility APIs
void loadXY(PointXY *r,
            uint32_t *source);                                                // load XY from memory (24 x 32-bits)
void storeXY(uint32_t *destination,
             PointXY *point);                                      // store XY to memory (24 x 32-bits)
void loadXYZZ(PointXYZZ *r,
              uint32_t *source);                                            // load XYZZ from memory (48 x 32-bits)
void storeXYZZ(uint32_t *destination,
               PointXYZZ *point);                                  // store XYZZ to memory (48 x 32-bits)
void storeAccumulatorXYZZ(uint32_t *destination,
                          AccumulatorXYZZ *accumulator);           // combination of getAccumulator and storeXYZZ
...
void normalizeXYZZ(PointXY *r,
                   PointXYZZ *point);                                         // r (affine point) = normalized(point)

// High level APIs
void scaleXY(PointXYZZ *r, PointXY *affinePoint, uint32_t *scalar,
             uint32_t bits);        // r (xyzz point) = affinePoint * scalar
void
scaleXYZZ(PointXYZZ *r, PointXYZZ *point, uint32_t *scalar, uint32_t bits);          // r (xyzz point) = point * scalar
void scaleByLambdaXY(PointXY *r,
                     PointXY *point);                                         // r (affine point) = lambda * affinePoint

void
lambdaQR(uint32_t *q, uint32_t *r, uint32_t *scalar);                                // q=scalar/lambda, r=scalar%lambda
